...
class LaneEngine:
    def __init__(self, trt_engine_path, input_size=(288, 800), image_size=(480, 640), num_lanes=2, num_grid=100, cls_num_per_lane=56, save=False, vis=False):
        # 생성자 초기화
    ...
    def run(self, frame):
        ...
        # **추가**: 활성화된 픽셀 수 로그 출력
        print(f"활성화된 차선 픽셀 수: {np.sum(lane_mask > 0)}")

        # **추가**: 차선 마스크 시각화
        if self.vis:
            # GUI 대신 저장하거나 로그만 출력
            cv2.imwrite("lane_mask_debug.png", lane_mask)  # 차선 마스크를 파일로 저장
            print("차선 마스크 저장 완료 (lane_mask_debug.png)")
