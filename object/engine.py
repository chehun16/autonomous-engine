...
class YOLOEngine:
    ...
    def infer(self, frame):
        """YOLO 모델로 추론 실행 및 결과 저장"""
        
        # 모델 추론 실행
        output = self.model(frame)  # YOLO 모델 추론

        base_folder = "results_yolo"
        date_folder = datetime.now().strftime("%Y%m%d_%H%M")  # 예: 0115
        result_folder = os.path.join(base_folder, date_folder)
        
        # 폴더가 없으면 생성
        os.makedirs(result_folder, exist_ok=True)
        
        # 폴더 안의 기존 파일 수를 세어 result_count로 설정
        result_count = len(os.listdir(result_folder))  # 해당 폴더에 있는 파일 수
        
        # 결과 이미지 파일명 생성 (예: result0.png, result1.png, ...)
        result_image_path = os.path.join(result_folder, f"result{result_count}.png")
        
        # 시각화된 추론 결과 이미지 저장
        result_image = output[0].plot()  # 시각화된 결과 반환
        cv2.imwrite(result_image_path, result_image)  # 이미지 저장
